...
print "#Accession\tDate\tAlias\tTotal_readsAfterTrim\tMapEff\tDupLev\tCpG\tCHG\tCHH\n";
while(<FILE>){
    next if /^#/;
    ...
    close REP;
    my $dup_rep = "$sample_name\_R1.fq.trimmed.paired1_bismark_bt2_pe.deduplication_report.txt";
    open DUP, $dup_rep or die "$dup_rep: $!";
    @report =  <DUP>;
    $rep = join("", @report);
    my ($dup) = $rep =~ /Total number duplicated alignments removed:\s+\d+\s+\((.*)%\)/; 
    close DUP;
    print "$acc\t$date\t$sample_name\t$tot_reads\t$map_eff\t$dup\t$lev_cpg\t$lev_chg\t$lev_chh\n";
}
close FILE;
