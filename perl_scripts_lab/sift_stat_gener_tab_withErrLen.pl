...
use strict;
########################
#In the beginning, there were some error occured while runing SIFT because gene length was to long. 
########################
my ($info, $sift_dir, $long_gene, $out) = @ARGV;
die usage() unless @ARGV == 4;
sub usage{
    my $die =<<DIE;
perl $0 <infor sift> <sift dir> <long gene err list> <out>
DIE
}

open GENE, $long_gene or die "$!";
my %long_gene_err;
while(<GENE>){
    chomp;
    my ($down, $id,$chg_pos, $ori_pos) = split;
    $long_gene_err{"$id\t$chg_pos"} = $ori_pos;
}
...
while(<INFO>){
    ...
    if(!-e $sift_res){
        ...
        $sift_res = "protein_seq_sift_tmp_sec/$id.SIFTprediction";
        if(-e $sift_res){
            ...
            while(my $res = <TMP>){
                ...
                my $ori_pos = $long_gene_err{"$id\t$codon_chg"};
                $rec_sift_res{"$id\t$ori_pos"} = "$class\t$score";
            }
        }else{
            print "$id\n";
            next;
