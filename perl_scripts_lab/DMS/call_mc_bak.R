...
call_mc <- function(tab, dep_cut, fdr, output){
    ...
    cat("Done\n")
    for(i in 1:nrow(t)){
        dep <- t[i,]$number_of_C + t[i,]$number_of_T;
        if(identical(t[i,]$chromosome,"chrM")){
            t <- t[-i,]
        }

        if(identical(t[i,]$chromosome,"chrC")){
            print(1);
            chloro_mc = chloro_mc + t[i,]$number_of_C;
            chloro_dep = chloro_dep + dep;
            t <- t[-i,]
        }
    }
...
call_mc(file_name, dep_cut, fdr, out.name)
