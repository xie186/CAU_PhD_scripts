...
call_mc <- function(tab, dep_cut, fdr, output){
    ...
    #err_rate
    #chr1    10000038        1       13      7.69230769230769
    t.chrC <- t[grepl("chrC", t[,1]),] 
    chloro_mc <- sum(t.chrC[,3])
    chloro_dep <- sum(t.chrC[,4]) + sum(t.chrC[,3])
    err_rate <- chloro_mc/chloro_dep;
    cat(date(), err_rate, chloro_mc, chloro_dep, "\n");
 
    t<-t[!(t[,4] > dep_cut_high | t[,4] < dep_cut_low), ] 
    ...
    p.val[ind1] <- lapply(ind1, function(x) binom.test(t[x, 3], t[x, 4], err_rate)$p.value);
    ...
    colnames(t) <- c("chr", "pos", "#c1","#dep", "lev",  "p.val", "qvalue", "METH")
    cat("Q cbind Done!\n\n");
