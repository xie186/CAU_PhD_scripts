...
my ($file, $var_dir, $popu_var) = @ARGV;

open VAR,$popu_var or die "$!";
my %geno_var;
while(<VAR>){
    chomp;
    next if /pos/;
...
while(<FILE>){
    ...
    open OUT, "+>VarScan_$stress\_$control\_$sample.popu_var.txt" or die "$!";
    open TEM, "cat $var_dir/VarScan_$stress\_$control.snp.Somatic.hc.fil.pass |" or die "$!";
    ...
    while(my $line = <TEM>){
         ...
         if(exists $geno_var{"$chr\t$pos"}){
             ...
             if($alt eq $alt1){
                 $popu_var ++;
                 print OUT "$_\t$chr\t$pos\t$ref\t$alt\t$res\t$geno_var{\"$chr\t$pos\"}\n";
             }
         }
    }
    close TEM;
    print "$_\t$var_tot\t$popu_var\n";
}

sub usage{
    my $die =<<DIE;
    perl *.pl <file> <variation dir> <popu_var>
DIE
}
