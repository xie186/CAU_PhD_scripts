draw_mth_dis_dif_ele_dif_RPKM5<-function(CG){
    cg<-read.table(CG);
    cg<-cg[order(cg[,2]),]
    max<-max(cg[,3],cg[,4],cg[,5],cg[,6],cg[,7]);
    
    plot(1:100,cg[cg[,1]=="upstream",][,3],type="l",xlim=c(0,700),ylim=c(0,max),xlab="Gene Model",ylab="Methylation level",col="DarkGreen",axes=FALSE)
    axis(2)
    axis(1,label=FALSE)
