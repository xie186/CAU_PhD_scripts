...
while(my $line=<POS>){
    ...
    chomp $line;
    my ($chr,$ele,$stt,$end,$strand,$gene,$fpkm)=(split(/\s+/,$line))[0,1,2,3,4,5,6];
    ...
    foreach($forw,$rev){
        ...
        while($row->fetch()){
            &cal($stt,$end,$strand,$tem_pos1,$lev,$ele,$fpkm);
        }
    }
}
...
close OUT;
sub cal{
    my ($stt,$end,$strand,$pos1,$methlev,$ele,$fpkm)=@_;
    if($fpkm>0){
        $fpkm="exp";
    }else{
        $fpkm="expno";
    ...
    $keys-=1 if $keys==100;
    $meth_forw{"$fpkm\t$ele\t$keys"}+=$methlev;
    $meth_forw_nu{"$fpkm\t$ele\t$keys"}++;
}
sub usage{
    my $die=<<DIE;
    perl *.pl <Tissue> <Forword> <Reverse> <GFF_exp && noexp [Low High TE TEno]> <OUTPUT>
    This is to get the methylation distribution througth gene for different types of gene.
DIE
}
