draw_TE_dis_gene_RPKM6<-function(S,contxt){
    ...
    plot(lowess(cc[,2],f=0.05),type="l",ylim=c(0,max),main=contxt,axes=FALSE,xlab="",ylab="",col="black")
    lines(lowess(cc[,3],f=0.05),col="DarkGreen")
    lines(lowess(cc[,4],f=0.05),col="red")
    lines(lowess(cc[,5],f=0.05),col="blue")
    lines(lowess(cc[,6],f=0.05),col="IndianRed4")
    lines(lowess(cc[,7],f=0.05),col="Chocolate")
