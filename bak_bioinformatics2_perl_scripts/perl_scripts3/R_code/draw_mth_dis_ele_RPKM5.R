draw_mth_dis_dif_ele_dif_RPKM5<-function(CG,bg,contxt){
    cg<-read.table(CG)
    bg<-read.table(bg)   ## to calculate the range of ylim
    cg<-cg[order(cg[,2]),]
    rang<-range(0,cg[,3],cg[,4],cg[,5],cg[,6],cg[,7],bg[,3],bg[,4],bg[,5],bg[,6],bg[,7]);
    
    plot(1:100,cg[cg[,1]=="upstream",][,3],type="l",xlim=c(0,700),ylim=rang,xlab="Gene Model",ylab="Methylation level(%)",col="DarkGreen",axes=FALSE,main=contxt)
    axis(2)
    axis(1,label=FALSE)
   ...
   lines(101:200,cg[cg[,1]=="first_exon",][,3],,col="DarkGreen")
   ...
   lines(101:200,cg[cg[,1]=="first_exon",][,7],col="Chocolate")
    ...
    lines(201:300,cg[cg[,1]=="first_intron",][,3],col="DarkGreen")
    ...
    lines(201:300,cg[cg[,1]=="first_intron",][,7],col="Chocolate")
     ...
     lines(301:400,cg[cg[,1]=="internal_exon",][,3],col="DarkGreen")
     ...
     lines(401:500,cg[cg[,1]=="internal_intron",][,3],col="DarkGreen")
     ...
     lines(501:600,cg[cg[,1]=="last_exon",][,3],col="DarkGreen")
     ...
     lines(601:700,cg[cg[,1]=="downstream",][,3],col="DarkGreen")
