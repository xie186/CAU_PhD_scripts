...
while(my $line=<POS>){
    ...
    chomp $line;
    my ($chr,$ele,$stt,$end,$strand,$name,$TE_or)=(split(/\s+/,$line));
    next if ($ele=~/mRNA/ ||$ele=~/CDS/ || $ele=~/gene/ || $end-$stt==0 || $ele =~ /stream/);
    $chr="chr".$chr;
    foreach($forw,$rev){
        ...
        while($row->fetch()){
            &cal($stt,$end,$strand,$tem_pos1,$lev,$ele,$TE_or);
        }
    }
}
...
close OUT;
sub cal{
    my ($stt,$end,$strand,$pos1,$methlev,$ele,$TE_or)=@_;
    ...
    $keys-=1 if $keys==100;
    $meth_forw{"$ele\t$TE_or\t$keys"}+=$methlev;
    $meth_forw_nu{"$ele\t$TE_or\t$keys"}++;
}
sub usage{
    my $die=<<DIE;
    perl *.pl <Tissue> <Forword> <Reverse> <GFF> <OUTPUT>
