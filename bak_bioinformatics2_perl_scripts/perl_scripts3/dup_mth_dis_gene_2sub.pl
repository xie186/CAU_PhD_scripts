...
#open DUP,"cut -f1,2,4,5 $dup |" or die "$!";
open DUP,"$dup" or die "$!";
while(my $line=<DUP>){
    print "$flag have been done\n" if $flag%5000==0;$flag++;
    chomp $line;
    my ($gene1,$sub_geno1,$gene2,$sub_geno2)=split(/\s+/,$line);
#    my $exp="exp";
#       $exp="expno" if($rpkm1==0 && $rpkm2==0);
    ####  Gene1   
    my %hash;
...
#    print "$chr,$stt,$end,$name,$strand\n";
    ...
    foreach(my $i=$stt-2000;$i<=$end+2000;++$i){
        if(exists $hash{"$chr\t$i"}){
            &cal($stt,$end,$strand,$i,$hash{"$chr\t$i"},$sub_geno1);
        }
    }
    %hash=();
    ...
    foreach(my $i=$stt-2000;$i<=$end+2000;++$i){
        if(exists $hash{"$chr\t$i"}){ 
            &cal($stt,$end,$strand,$i,$hash{"$chr\t$i"},$sub_geno2);
        }
    }
    %hash=();
}
