draw_mth_dis_ele_RPKM10<-function(CG,CG_out){
#   pdf(OUT)
   cg<-read.table(CG)
   cg<-cg[order(cg[,2]),]
   cg_out<-read.table(CG_out)

   max<-max(cg[,3],cg[,4],cg[,5],cg[,6],cg[,7],cg[,8],cg[,9],cg[,10],cg[,11],cg[,12],cg_out[,3],cg_out[,4],cg_out[,5],cg_out[,6],cg_out[,7],cg_out[,8],cg_out[,9],cg_out[,10],cg_out[,11],cg_out[,12]);
    
    plot(1:100,cg[cg[,1]=="upstream",][,3],type="l",xlim=c(0,700),ylim=c(0,max),xlab="Gene Model",ylab="Methylation level",col="black")
   ...
   lines(101:200,cg[cg[,1]=="first_exon",][,3],,col="black")
   ...
   lines(101:200,cg[cg[,1]=="first_exon",][,8],col="purple")
    ...
    lines(201:300,cg[cg[,1]=="first_intron",][,3],col="black");
    ...
    lines(201:300,cg[cg[,1]=="first_intron",][,12],col="DarkOrange")
     ...
     lines(301:400,cg[cg[,1]=="internal_exon",][,3],col="black");
    ...
    lines(301:400,cg[cg[,1]=="internal_exon",][,12],col="DarkOrange")
     ...
     lines(401:500,cg[cg[,1]=="internal_intron",][,3],col="black");
    ...
    lines(401:500,cg[cg[,1]=="internal_intron",][,12],col="DarkOrange")
     ...
     lines(501:600,cg[cg[,1]=="last_exon",][,3],col="black");
    ...
    lines(501:600,cg[cg[,1]=="last_exon",][,12],col="DarkOrange")
     ...
     lines(601:700,cg[cg[,1]=="downstream",][,3],col="black")
