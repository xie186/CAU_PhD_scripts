...
my ($region,$forw,$rev,$chg_forw,$chg_rev,$out,$tissue)=@ARGV;
...
while(<REGION>){
    ...
    my ($c_nu,$methlev)=(0,0);
        ...
        $row=$dbh->prepare(qq(select * from $chg_forw where chrom="$chr" and pos1>=$stt and pos1<=$end));
           $row->execute();
           ($chrm,$pos1,$pos2,$depth,$lev)=(0,0,0,0,0);
           $row->bind_columns(\$chrm,\$pos1,\$pos2,\$depth,\$lev);
         while($row->fetch()){
        ...
        $row=$dbh->prepare(qq(select * from $chg_rev where chrom="$chr" and pos1>=$stt and pos1<=$end));
           $row->execute();
           ($chrm,$pos1,$pos2,$depth,$lev)=(0,0,0,0,0);
           $row->bind_columns(\$chrm,\$pos1,\$pos2,\$depth,\$lev);
         while($row->fetch()){
...
sub usage{
    my $die=<<DIE;
    Usage:perl *.pl <Regions > <Forw_CpG> <Rev_CpG> <Forw_CHG> <Rev_CHG> <OUT1> <Tissue>
    OUTPUT:
