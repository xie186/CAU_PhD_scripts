...
sub methy{
    ...
    if($strand eq '+'){
        for(my $i=$pos1-100;$i<=$pos1+100;++$i){
            if(exists $hash{"$chr\t$i\tforw"}){
                $methforw_TSS{$i-$pos1}+=$hash{"$chr\t$i\tforw"};
                $methforw_TSS_nu{$i-$pos1}++;
            }elsif(exists $hash{"$chr\t$i\trev"}){
                $methrev_TSS{$i-$pos1}+=$hash{"$chr\t$i\trev"};
       ...
       for(my $i=$pos2-100;$i<=$pos1+100;++$i){
            if(exists $hash{"$chr\t$i\tforw"}){
                $methforw_TSS{$pos2-$i}+=$hash{"$chr\t$i\tforw"};
                $methforw_TSS_nu{$pos2-$i}++;
            }elsif(exists $hash{"$chr\t$i\trev"}){
                $methrev_TSS{$pos2-$i}+=$hash{"$chr\t$i\trev"};
