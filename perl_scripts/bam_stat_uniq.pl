...
print "#Accession\tDate\tSampleType\tTotalReads\tMappedReads\tMappingPercentage\tProperPair\tProperPairPercentage\n";
while(<FILE>){
    next if /^#/;
    ...
    open TEM, "$acc\_$sample_name\_$date\_srt.stat" or die "$acc\_$sample_name\_$date\_srt.stat:$!";
    my @stat = <TEM>;
    ...
    print "$acc\t$date\t$sample_name\t$tot_reads\t$mapped_reads\t$map_eff\t$proper_pair\t$proper_pair_eff\n"; 
}
