...
#            par(mfrow=c(3,1),mar=c(2,3,3,1))
            ...
            lines((sam3[sam3[,1] == chr,][,3]-50000)/1000000 , sam3[sam3[,1] == chr,][,7],col = rgb(125,96,160,max = 255)) 
            lines((sam4[sam4[,1] == chr,][,3]-50000)/1000000 , sam4[sam4[,1] == chr,][,7],col = rgb(70,170,197,max = 255))

            mtext(c("Methylation Level"),side=2,line=2)

            legend(maxi -6, maxi_y*0.9,c("156","204","ros1","c24"), cex=0.8, col=c(rgb(74,126, 187,96,160,max = 255), rgb(190,75,72,max = 255), rgb(125,96,160,max = 255), rgb(70,170,197,max = 255)), lty=1); 
            axis(2)
            axis(1)
            box()
