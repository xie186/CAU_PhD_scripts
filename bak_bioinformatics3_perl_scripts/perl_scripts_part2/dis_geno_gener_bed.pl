...
while(<REG>){
    ...
    push @{$hash{"$chr\t$mid"}},(-1,"$chr\t$stt\t$end");
}
...
while(<GFF>){
    ...
    for(my $i = $stt;$i<=$end;++$i){
         if(exists $hash{"chr$chr\t$i"}){
              ...
              ${$hash{"chr$chr\t$i"}}[0] = $fea_value if $fea_value > ${$hash{"chr$chr\t$i"}}[0];
         }
    }
}
...
foreach(keys %hash){
    my $ele;
       $ele = "Intergenic" if ${$hash{$_}}[0] == -1;
       $ele = "Downstream" if ${$hash{$_}}[0] == 0;
       $ele = "Upstream" if ${$hash{$_}}[0] == 1;
       $ele = "Intron" if ${$hash{$_}}[0] == 2;
       $ele = "Exon" if ${$hash{$_}}[0] == 3;
    print "${$hash{$_}}[1]\t$ele\n";
}
