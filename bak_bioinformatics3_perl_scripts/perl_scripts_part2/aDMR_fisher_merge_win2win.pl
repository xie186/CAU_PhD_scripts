...
while(my $line=<DMR>){
    my ($chr1,$stt1,$end1,$lev_bmb1,$lev_bmm1,$lev_mbb1,$lev_mbm1)=(split(/\s+/,$line))[0,1,2,6,10,14,18];
    PATH:{
        $line=<DMR>;
        if(!$line){
            print OUT "$chr1\t$stt1\t$end1\n";
        }else{
            my ($chr2,$stt2,$end2,$lev_bmb2,$lev_bmm2,$lev_mbb2,$lev_mbm2)=(split(/\s+/,$line))[0,1,2,6,10,14,18];
...
#                 print "ooa\n$chr1,$stt1,$end1,$lev_bmb1,$lev_bmm1,$lev_mbb1,$lev_mbm1\n$chr2,$stt2,$end2,$lev_bmb2,$lev_bmm2,$lev_mbb2,$lev_mbm2\n" if (($lev_bmb1 > $lev_bmm1 && $lev_bmb2 < $lev_bmm2) || ($lev_bmb1 < $lev_bmm1 && $lev_bmb2 > $lev_bmm2));
                 if (($lev_bmb1 > $lev_bmm1 && $lev_bmb2 < $lev_bmm2) || ($lev_bmb1 < $lev_bmm1 && $lev_bmb2 > $lev_bmm2)){
                     $line = <DMR>;
                     ($chr1,$stt1,$end1,$lev_bmb1,$lev_bmm1,$lev_mbb1,$lev_mbm1)=(split(/\s+/,$line))[0,1,2,6,10,14,18]; 
                 }else{
                     $end1=$end2;
                 }
                 redo PATH;
