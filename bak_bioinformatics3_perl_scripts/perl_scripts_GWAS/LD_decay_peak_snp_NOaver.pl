...
my ($hapmap, $peak_snp, $win ,$inbred_num, $r2_cut , $out) = @ARGV;
sub usage{
    my $die =<<DIE;
    perl *.pl <hapmap> <peak snp> <win size> <inbred line nu> <r2 cut> <OUT>
DIE
}
    ...
    if(!-e "./chr_bin/$chr\_$pos.res"){
        ...
        for(my $i = 0; $i < $snp_nu ; ++ $i){
            ...
            for(my $j = $i + 1 ; $j < $snp_nu ; ++$j){
                ...
                my ($r2) = &r2($geno1,$geno2);
                my $dis = abs($pos2 - $pos1);
                print TEM "$chr\t-\t-\t$dis\t$r2\n";
            }
        }
    }
...
dis_ld_decay <- uniroot(f, interval = c(0,20000000))\$root
EOF
    my $run = $R ->run($cmd);
