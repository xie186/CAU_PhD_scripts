...
my %hash;
while (<T>){
    my ($coor,$maf,$log)=split(/\s+/,$_);
        $coor =~ s/chr//g;
       $log = abs(log($log)/log(10));
       $log = sprintf "%0.2f", $log;
    my ($chr,$pos) = split(/_/,$coor);
    `rm tem_dir_manhattan_plot/temp-$p_log-chr$chr` if -e ("./tem_dir_manhattan_plot/temp-$p_log-chr$chr" && !exists $hash{"temp-$p_log-chr$chr"});
    open T1, "+>>tem_dir_manhattan_plot/temp-$p_log-chr$chr";
    $hash{"temp-$p_log-chr$chr"} ++;
    print T1 "$n\t$log\n";
    close T1;
    if($max{$chr} < $n){
        $max{$chr} = $n;
