...
my ($geno_len,$chrom,$bim_prefix,$win,$reg_stt,$reg_end) = @ARGV;
my $bin = 100;
...
for(my $i = 1;$i<=$nu;++$i){
    ...
    next if ($stt < $reg_stt || $stt > $reg_end);
    $chrom =~ s/chr//g if $chrom=~/chr/;
    my $win_kb = $win/1000;
    my $win_left = $win -1;
    `/NAS2/jiaoyp/software/plink-1.07-x86_64/plink --bfile $bim_prefix --noweb --r2 --ld-window-kb 200000 --ld-window 199999 --ld-window-r2 0 --maf 0.05 --chr $chrom --from-bp $stt --to-bp $end --out ./ld_cal/chr$chrom\_$stt\_$end\_win$win` if !-e "./ld_cal/chr$chrom\_$stt\_$end\_win$win.ld";
