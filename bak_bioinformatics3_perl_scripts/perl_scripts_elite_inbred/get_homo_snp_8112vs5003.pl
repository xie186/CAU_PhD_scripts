...
while(<F>){
	...
	next if ($dp5003 < 5 || $dp8112 < 5);
	next if ($gt5003 eq $gt8112);
	next if ($gt5003 eq "0/1" || $gt8112 eq "0/1");
...
sub usage{
    ...
    criteria:1) depth in 8112 and 5003, no less than 5; 2) either allele eq zero; 3) 
DIE
}
