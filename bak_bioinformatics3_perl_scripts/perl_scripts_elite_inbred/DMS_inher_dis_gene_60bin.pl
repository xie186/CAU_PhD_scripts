...
foreach(sort keys %count_dms){
    my $aver_dms_num_per_1k_sites = ${$count_dms{$_}}[0] * 100 / ${$count_dms{$_}}[1];
    $_=~s/prom/-1/;
    $_=~s/body/0/;
    $_=~s/term/1/;
...
sub cal{
    ...
    if($judge eq "DMS_inheritance"){
        if($dms_bw_parents_jug eq "S"){
            ${$count_dms{$keys}}[0] ++;
        }
    }else{
        die "Wrong type!\n";
...
sub usage{
    my $die=<<DIE;
    perl *.pl <qvalue results> <Gene position> <judge [DMS_inheritance> <OUTPUT>
