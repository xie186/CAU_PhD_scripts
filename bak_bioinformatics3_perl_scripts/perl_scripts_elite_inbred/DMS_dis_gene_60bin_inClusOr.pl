...
my ($dms,$ge_pos, $clus, $judge, $out)=@ARGV;
my $BIN = 60;
...
while(<DMS>){
    ...
    my $in_clus = "CLUS";
    if(exists $clus_no{"$chr\t$stt"}){
        $in_clus = "notCLUS";
    }
    @{$dms_pos{"$chr\t$stt"}} = ($qval, $dms_bw_parents_jug, $in_clus);
}
...
foreach(sort keys %count_dms){
    my $aver_dms_num_per_1k_sites_notCLUS = ${$count_dms{$_}}[0] * 1000 / ${$count_dms{$_}}[2];
    my $aver_dms_num_per_1k_sites_CLUS = ${$count_dms{$_}}[1] * 1000 / ${$count_dms{$_}}[2];
    $_=~s/prom/-1/;
    $_=~s/body/0/;
    $_=~s/term/1/;
    print OUT "$_\t$aver_dms_num_per_1k_sites_notCLUS\t$aver_dms_num_per_1k_sites_CLUS\n";
}
close OUT;

sub cal{
    my ($stt,$end,$strand,$pos1,$qval, $dms_bw_parents_jug, $in_clus) = @_;
...
sub usage{
    my $die=<<DIE;
    perl *.pl <qvalue results> <Gene position> <clusOrNot> <judge [pDMS, sDMS, DMS]> <OUTPUT>
