...
while(my $line=<DUP>){
    ...
    chomp $line;
    my ($gene1,$sub_geno1,$rpkm1,$gene2,$sub_geno2,$rpkm2)=split(/\s+/,$line);
    my ($type1,$type2)=("paired1_rpkm_big","paired2_rpkm_small");
    if($rpkm2>$rpkm1){
        ($type1,$type2)=("paired2_rpkm_small","paired1_rpkm_big");
    }
...
#    print "$chr,$stt,$end,$name,$strand\n";
    ...
    foreach(my $i=$stt-2000;$i<=$end+2000;++$i){
        if(exists $hash{"$chr\t$i"}){
            &cal($stt,$end,$strand,$i,$hash{"$chr\t$i"},$type1);
        }
    }
    %hash=();
    ...
    foreach(my $i=$stt-2000;$i<=$end+2000;++$i){
        if(exists $hash{"$chr\t$i"}){ 
            &cal($stt,$end,$strand,$i,$hash{"$chr\t$i"},$type2);
        }
    }
    %hash=();
}
...
close OUT;
system(qq(sort -k2,2n -k3,3n $out >$out.srt));
