...
#  pdf(OUT);
    ...
    chh<-read.table(CHH);
    chh<-chh[order(chh[,2]),]
    max<-max(cg[,4],chg[,4],chh[,4]);
    min<-min(cg[,4],chg[,4],chh[,4]);
 
    plot(1:100,cg[cg[,1]=="upstream",][,4],type="l",xlim=c(0,700),ylim=c(min,max),col="red",xlab="Gene Model",ylab="r of Spearman' Correlation"); 
