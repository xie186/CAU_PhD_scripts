...
while(<EXP>){
    chomp;
    my ($gene,$rpkm,$cpg_oe,$te)=(split)[0,3,4,5];
    @{$hash{$gene}}=($rpkm,$cpg_oe,$te);
}
...
while(<GFF>){
    ...
    my @aa=split;
    if(${$hash{$name}}[1]<0.9 && ${$hash{$name}}[2] eq "TE"){
        print OUT1 "$chr\t$ele\t$pos1\t$pos2\t$strand\t$name\t${$hash{$name}}[0]\n";
    }elsif(${$hash{$name}}[1]<0.9 && ${$hash{$name}}[2] eq "NO"){
        print OUT2 "$chr\t$ele\t$pos1\t$pos2\t$strand\t$name\t${$hash{$name}}[0]\n";
    }elsif(${$hash{$name}}[1]>=0.9 && ${$hash{$name}}[2] eq "TE"){
...
sub usage{
    my $die=<<DIE;
    \nUsage:perl *.pl <EXP> <GFF > <Low CpG && TE >  <Low CpG && noTE> <High CpG && TE > <High CpG && noTE>\n
DIE
}
