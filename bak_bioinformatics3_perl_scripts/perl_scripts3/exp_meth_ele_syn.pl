...
my ($gff, $spec, $gene_region, $BIN, $syn ,$tissue, $forw, $rev, $out) = @ARGV;
...
sub usage{
    print <<DIE;
    perl *.pl <FGS GFF file> <spcies ara rice or zm> <Gene element> <Bin number> <Syntenic genes> <Tissue> <OT> <OB>  <OUT>
DIE
    exit 1;
}
