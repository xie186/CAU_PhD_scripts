...
foreach my $tem_cmp(@cmp){
    ...
    while(<CMP>){
        ...
        my $fold_chg = $mth_lev2/($mth_lev1 + 0.00000001);
        if($p_val_adj < 0.01 && ($fold_chg > $cutoff || $fold_chg < 1/$cutoff)){
...
sub usage{
    ...
    perl *.pl <cmp [156,204]> <context[156,205]> <cut [1.5]> <OUT>
