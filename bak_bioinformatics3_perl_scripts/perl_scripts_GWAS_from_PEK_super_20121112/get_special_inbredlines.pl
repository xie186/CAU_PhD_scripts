...
my ($snp_vcf,$inbred_cau,$mis_cut,$maf_cut,$out) = @ARGV;
open T, $inbred_cau or die "$!";
my %name;
while (<T>){
    chomp;
...
sub usage{
    ...
    we use this to delete some inbred lines in the snp data [/datastore/workspace/caulai/jiao/GWAS_2012_June/3_cat-snp/delete-inbred.pl]
DIE
    exit 1;
}
