...
print "Transcripts\tChr\tSNP_relative_position\tBM_B\tBM_M\tMB_B\t\tMB_M\tPcalueBM\t\tPvalueMB\tstart\tend\tmat_biasBM\tmat_bias_MB\t>2:1\t>5:1\tsnp\n";
my $snp;
my $seqn;
while($seqn=<SEQ>){
     chomp $seqn;
     my ($chr_seq,$up,$down)=$seqn=~/>IES_(chr\d+)_(\d+)_(\d+)/;
#     print "$chr_seq\n";
     my $seq=<SEQ>;
     open SNP,$ARGV[0] or die "$!";
     while ($snp=<SNP>){
...
#             print "here\n";
             ...
             my $bias1=$b73_mnu/($b73_mnu+$mo_pnu);
             my $bias2=$mo_mnu/($mo_mnu+$b73_pnu);
             my @array=(0);
