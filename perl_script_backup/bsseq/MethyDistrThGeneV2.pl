...
while(my $ge=<GENE>){
    ...
    while(my $ac=<ACGT>){
        ...
        if( $judg_chr!=0){
            next;
        }elsif($mepos>$gepos[1]-5000 && $mepos<$gepos[2]+5000 && $read>=5 && $lev>0){
...
sub  distri{
    ...
    if($strand eq "+"){
        for(my $i=0;$i<150;++$i){
           my $end;
           if($i==99){
                $end=$gestt-5000+$i*100+$residue;
           }else{
                $end=$gestt-5000+$i*100;
           }                                                         #  print "$mepos\t$end\t$geend\+\($i-1\)\*100\n";
           if($mepos>=$gestt-5000+$i*100 && $mepos<=$gestt-5000+($i+1)*100){
        ...
        for(my $i=149;$i>=0;--$i){
            my $end;
            if($i==99){
                $end=$geend+5000-(149-$i)*100+$residue;
            }else{
                $end=$geend+5000-(149-$i)*100;
            }
            if($mepos>=$end+5000-(150-$i)*100 && $mepos<=$end+5000-(149-$i)*100){
                ${hash{$i}}[0]++ if $text eq "CG";
                ${hash{$i}}[1]++ if $text eq "CHG";
                ${hash{$i}}[2]++ if $text eq "CHH";
            }
